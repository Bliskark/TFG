...
<?php $__env->startSection('title', 'Usuarios y Eventos'); ?>
...
<div class="bg-gray-800 p-3 rounded-t-lg flex items-center space-x-2 border-b-2 border-gray-700">
  ...
  <a href="<?php echo e(route('admin.usuarios.create')); ?>" class="pokedex-btn bg-indigo-600 text-sm px-3">
    + Crear Usuario
  </a>
...
<div class="p-4">
  <table class="w-full border-collapse">
    ...
    <tbody>
      ...
      <tr class="border-b border-gray-200 hover:bg-gray-100 transition duration-150">
        <td class="py-3 px-3 text-base"><?php echo e($u->email); ?></td>
        <td class="py-3 px-3">
          <span class="px-2 py-1 border-2 text-sm font-bold
            <?php if($u->rol == 'admin'): ?> bg-red-100 text-red-800 border-red-400
            <?php elseif($u->rol == 'lider'): ?> bg-green-100 text-green-800 border-green-400
            <?php elseif($u->rol == 'alto_mando'): ?> bg-purple-100 text-purple-800 border-purple-400
            <?php elseif($u->rol == 'campeon'): ?> bg-yellow-100 text-yellow-800 border-yellow-400
            <?php else: ?> bg-blue-100 text-blue-800 border-blue-400
            <?php endif; ?>">
            <?php echo e($u->rol); ?>
        ...
        <td class="py-3 px-3">
          <div class="flex space-x-2">
            <a href="<?php echo e(route('admin.usuarios.edit', $u->id)); ?>" class="pokedex-btn bg-blue-500 text-sm px-3">Editar</a>
            <a href="<?php echo e(route('admin.usuarios.equipo', $u->id)); ?>" class="pokedex-btn bg-green-500 text-sm px-3">Equipo</a>
            <form action="<?php echo e(route('admin.usuarios.destroy', $u->id)); ?>" method="POST" onsubmit="return confirm('¿Eliminar este usuario?')" class="inline">
              <?php echo csrf_field(); ?>
  ...
  <div class="mt-4 text-right text-gray-700">
    <span>Total de usuarios: <?php echo e(count($usuarios)); ?></span>
  </div>
</div>
...
<div class="mt-10 p-4 bg-white rounded-lg shadow">
  <div class="flex justify-between items-center mb-4">
    ...
    <a href="<?php echo e(route('admin.eventos.create')); ?>" class="pokedex-btn bg-yellow-600 text-sm px-3">
      + Crear Evento
    </a>
  </div>
  ...
  <table class="w-full border-collapse mb-4">
    ...
    <tbody>
      ...
      <tr class="border-b border-yellow-300 hover:bg-yellow-100 transition duration-150">
        <td class="py-3 px-3"><?php echo e($evento->name); ?></td>
        <td class="py-3 px-3"><?php echo e($evento->description); ?></td>
        <td class="py-3 px-3"><?php echo e($evento->created_at->format('d/m/Y')); ?></td>
        <td class="py-3 px-3"><?php echo e($evento->tipo); ?></td>
        <td class="py-3 px-3"><?php echo e($evento->efecto); ?></td>
        <td class="py-3 px-3">
          <div class="flex space-x-2">
            <a href="<?php echo e(route('admin.eventos.edit', $evento->id)); ?>" class="pokedex-btn bg-yellow-500 text-sm px-3">Editar</a>
            <form action="<?php echo e(route('admin.eventos.destroy', $evento->id)); ?>" method="POST" onsubmit="return confirm('¿Eliminar este evento?')" class="inline">
              <?php echo csrf_field(); ?>
              <?php echo method_field('DELETE'); ?>
              <button type="submit" class="pokedex-btn bg-red-600 text-sm px-3">Borrar</button>
            </form>
  ...
  <div class="mt-4 text-right text-gray-700">
    <span>Total de eventos: <?php echo e(count($eventos)); ?></span>
  </div>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Blisk\Desktop\pokeCombates\resources\views/admin/usuarios/index.blade.php ENDPATH**/ ?>