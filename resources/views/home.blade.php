...
@section('title', 'PC de ' . (Auth::check() ? Auth::user()->email : 'Alguien'))
    ...
    <div class="w-full max-w-md bg-red-600 rounded-lg shadow-xl overflow-hidden border-6 border-gray-800">
        ...
        <div class="pokedex-screen m-4 p-5 rounded-lg border-4 border-gray-800">
            <div class="flex flex-col space-y-3">
                @guest
                    <a href="{{ route('login') }}" class="pokedex-btn bg-blue-500 action-btn">
                        Iniciar Sesión
                    </a>
                @endguest
                
                <a href="{{ route('statistics') }}" class="pokedex-btn bg-green-500 action-btn">
                    Estadísticas
                </a>
                
                <a href="{{ route('pokedex.index') }}" class="pokedex-btn bg-red-500 action-btn">
                    Pokédex
                </a>
                
                <a href="{{ route('equipo.index') }}" class="pokedex-btn bg-yellow-500 action-btn">
                    Equipo
                </a>
                
                <a href="{{ route('battle.show') }}" class="pokedex-btn bg-purple-500 action-btn">
                    Combatir
                </a>
                
                @auth
                    @if(Auth::user()->rol === 'admin')
                        <a href="{{ route('admin.usuarios.index') }}" class="pokedex-btn bg-gray-800 action-btn text-white">
