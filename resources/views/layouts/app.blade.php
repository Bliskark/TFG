...
<head>
  ...
  <title>@yield('title', 'PokeCombates')</title>
  <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css2?family=VT323&family=Press+Start+2P&display=swap" rel="stylesheet">
  <link rel="icon" href="{{ asset('pokeball.png') }}" type="image/png">
      ...
      <div class="container mx-auto p-2 sm:p-4 nav-container">
        <div class="flex justify-between items-center">
          ...
          <div class="flex items-center space-x-2 sm:space-x-4">
            ...
            <a href="{{ route('home') }}" class="title-font text-lg sm:text-xl tracking-wide truncate max-w-xs">PokeCombates</a>
          </div>
          ...
          <div class="hidden md:flex items-center space-x-2 lg:space-x-4">
            <a href="{{ route('statistics') }}" class="nav-link font-medium text-sm lg:text-base">Estadísticas</a>
            <a href="{{ route('pokedex.index') }}" class="nav-link font-medium text-sm lg:text-base">Pokédex</a>
            <a href="{{ route('battle.show') }}" class="nav-link font-medium text-sm lg:text-base">Combatir</a>
            
            @auth
              <a href="{{ route('equipo.index') }}" class="nav-link font-medium text-sm lg:text-base">Equipo</a>
            ...
            @else
              <a href="{{ route('login') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 sm:px-4 sm:py-2 rounded-md pokedex-btn action-btn text-sm lg:text-base">
                Login
              </a>
              <a href="{{ route('register') }}" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 sm:px-4 sm:py-2 rounded-md pokedex-btn action-btn text-sm lg:text-base">
                Register
              </a>
            @endauth
          </div>
        ...
        <div id="mobile-menu" class="md:hidden mt-2 space-y-1 overflow-hidden">
          <a href="{{ route('statistics') }}" class="block py-3 px-4 hover:bg-yellow-300 hover:text-gray-800 rounded transition-all">Estadísticas</a>
          <a href="{{ route('pokedex.index') }}" class="block py-3 px-4 hover:bg-yellow-300 hover:text-gray-800 rounded transition-all">Pokédex</a>
          <a href="{{ route('battle.show') }}" class="block py-3 px-4 hover:bg-yellow-300 hover:text-gray-800 rounded transition-all">Combatir</a>
          
          @auth
            <a href="{{ route('equipo.index') }}" class="block py-3 px-4 hover:bg-yellow-300 hover:text-gray-800 rounded transition-all">Equipo</a>
          ...
          @else
            <a href="{{ route('login') }}" class="block py-3 px-4 bg-blue-500 text-white rounded hover:bg-blue-600 transition-all">Login</a>
            <a href="{{ route('register') }}" class="block py-3 px-4 bg-green-500 text-white rounded hover:bg-green-600 transition-all mt-2">Register</a>
          @endauth
        </div>
      </div>
    </nav>
    ...
    <div class="container mx-auto px-2 sm:px-4 py-2">
      ...
      @if(session('error'))
        <div class="alert bg-red-100 border-red-500 text-red-800">
          <div class="flex items-start">
            <span class="mr-2 flex-shrink-0">❌</span>
            <p class="flex-grow">{{ session('error') }}</p>
      ...
      @if(session()->has('errors') && session('errors') instanceof \Illuminate\Support\ViewErrorBag)
        @if(session('errors')->any())
          <div class="alert bg-red-100 border-red-500 text-red-800">
            <div class="flex items-start">
              <span class="mr-2 flex-shrink-0">❌</span>
              <p class="flex-grow">{{ session('errors')->first() }}</p>
              <button class="flex-shrink-0 ml-2 focus:outline-none text-red-800 hover:text-red-600" onclick="this.parentNode.parentNode.remove()">×</button>
            </div>
          </div>
        @endif
      ...
      <div class="pokedex-border bg-red-500 flex-1 flex flex-col">
        <div class="pokedex-screen m-2 sm:m-4 p-3 sm:p-6 flex-1 flex flex-col">
          <div class="mb-2 sm:mb-4 flex justify-between items-center">
            <h1 class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-800 truncate">@yield('title', 'PokeCombates')</h1>
