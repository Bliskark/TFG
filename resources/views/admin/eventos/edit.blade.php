...
@section('content')
  <div class="max-w-3xl mx-auto bg-white bg-opacity-90 rounded-xl shadow-lg border-2 border-gray-700 overflow-hidden">
    ...
    <form action="{{ route('admin.eventos.update', $evento->id) }}" method="POST" class="p-4 sm:p-6">
      @csrf
      @method('PUT')
      ...
      <div class="mb-5">
        ...
        <div class="relative">
          <input type="text" name="name" id="name"
                value="{{ old('name', $evento->name) }}"
                class="w-full border-2 border-gray-700 rounded-lg p-3 bg-green-100 focus:bg-green-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all @error('name') border-red-500 bg-red-100 @enderror">
      ...
      <div class="mb-5">
        ...
        <div class="relative">
          <textarea name="description" id="description" rows="4"
                    class="w-full border-2 border-gray-700 rounded-lg p-3 bg-green-100 focus:bg-green-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all @error('description') border-red-500 bg-red-100 @enderror">{{ old('description', $evento->description) }}</textarea>
          <div class="absolute right-3 top-3">
            <div class="w-3 h-3 rounded-full bg-gray-500"></div>
          </div>
        </div>
      ...
      <div class="mb-5">
        ...
        <div class="relative">
          <select name="tipo" id="tipo"
                  class="w-full border-2 border-gray-700 rounded-lg p-3 bg-green-100 focus:bg-green-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all appearance-none @error('tipo') border-red-500 bg-red-100 @enderror">
            <option value="">-- Selecciona un tipo --</option>
            <option value="misterioso" {{ old('tipo', $evento->tipo) == 'misterioso' ? 'selected' : '' }}>Misterioso ❓</option>
            <option value="curar" {{ old('tipo', $evento->tipo) == 'curar' ? 'selected' : '' }}>Curar 💊</option>
            <option value="capturar" {{ old('tipo', $evento->tipo) == 'capturar' ? 'selected' : '' }}>Capturar 🏆</option>
          </select>
      ...
      <div class="mb-6">
        ...
        <div class="relative">
          <input type="text" name="efecto" id="efecto"
                value="{{ old('efecto', $evento->efecto) }}"
                class="w-full border-2 border-gray-700 rounded-lg p-3 bg-green-100 focus:bg-green-50 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all @error('efecto') border-red-500 bg-red-100 @enderror">
          <div class="absolute right-3 top-3">
            <div class="w-3 h-3 rounded-full bg-gray-500"></div>
          </div>
        </div>
