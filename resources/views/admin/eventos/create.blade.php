...
<div class="max-w-2xl mx-auto p-2">
  ...
  <div class="bg-red-600 rounded-lg shadow-lg p-4 border-4 border-gray-800">
    ...
    <div class="bg-gray-100 rounded-lg p-5 border-2 border-gray-800">
      ...
      @if(session('errors') instanceof \Illuminate\Support\ViewErrorBag && session('errors')->any())
      ...
      <form action="{{ route('admin.eventos.store') }}" method="POST">
        ...
        <div class="mb-4">
          ...
          <textarea name="description" id="description" rows="3"
                  class="w-full border-2 border-gray-400 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-300">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div class="bg-yellow-100 p-3 rounded-lg border-2 border-yellow-400">
            ...
            <select name="tipo" id="tipo" required
              ...
              <option value="misterioso"  {{ old('tipo')=='misterioso' ? 'selected' : '' }}>Misterioso</option>
              <option value="curar"       {{ old('tipo')=='curar'      ? 'selected' : '' }}>Curar</option>
              <option value="capturar"    {{ old('tipo')=='capturar'   ? 'selected' : '' }}>Capturar</option>
            </select>
          </div>
          <div class="bg-blue-100 p-3 rounded-lg border-2 border-blue-400">
            ...
            <input type="text" name="efecto" id="efecto" value="{{ old('efecto') }}"
                class="w-full border-2 border-gray-400 bg-white rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-300"
